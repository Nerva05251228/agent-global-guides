...
description: Plan, dispatch, monitor, and verify subagents when Claude is the primary agent, including Claude-to-Codex and Claude-to-Claude routing, mandatory Codex web-search delegation, task specs, read-only/editable invocation patterns, stream-json monitoring, timeout handling, retries, fallback, and permission boundaries. Use when Claude needs to delegate backend work, frontend work, implementation, review, search, parallel workstreams, or cross-agent execution.
---
...
- When a task involves web search, latest information, external official docs, prices, laws, versions, news, online fact-checking, or information that may change over time, use a single-turn Codex subagent for search. The Codex search task must search/verify/summarize only, avoid repository edits, include source links, distinguish source-confirmed facts from inference, and return `verified`, `failed with reason`, or `not verified`.
...
If a task strongly calls for independent subagent implementation, review, or analysis but Claude handles it fully directly, record the reason in the authoritative plan or state it before proceeding.
