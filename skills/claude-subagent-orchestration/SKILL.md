...
description: Use when Claude is primary and delegation, independent review, required web search, parallel workstreams, or cross-agent execution may materially improve the result.
---
...
The primary Claude agent remains responsible for planning, review, integration, dangerous operations, final verification, and acceptance. Never trust a subagent's self-report alone. Before accepting work, independently:
...
- Prefer Codex subagents for backend work, infrastructure, repository edits, refactors, migrations, build/test loops, mechanical migrations, log analysis, full-codebase search, and process or service management.
- Prefer Claude directly or Claude subagents for frontend product/design/development work, including UI/UX design, component implementation, layout and styling, interaction behavior, browser-facing copy, frontend review, and image inspection through the Claude SDK path.
...
- These are routing preferences. Claude may work directly when dispatch adds no material value, the task is small or tightly coupled to primary-session state, or the primary must retain control.
...
Prefer the platform's built-in task/subagent controls for dispatch, monitoring, cancellation, timeout, and result collection. Use CLI invocation only when built-in controls are unavailable or the task explicitly requires a CLI process. Linux/macOS may use the POSIX examples below. In Windows PowerShell, pipe `Get-Content -Raw` into `codex`, use `Tee-Object` for the JSONL log, and keep paths native or quoted; do not assume Bash, `/tmp`, `timeout`, `tail`, `sed`, or `ps` exists.
...
## Calling Claude Subagents

Use built-in Claude task/subagent controls first. For CLI fallback, the examples below use POSIX shell syntax for Linux. On macOS, prefer the built-in timeout because GNU `timeout` is not installed by default. On Windows PowerShell, replace input/output redirection with `Get-Content -Raw ... | & claude ... 2>&1 | Tee-Object -FilePath ...`, and enforce the hard timeout with the built-in task control or PowerShell process/job APIs.
...
- Fallback to the primary Claude agent, a Codex subagent, or another Claude subagent only after the process exits with failure, the stream log has no activity for 90-120 seconds, repeated API retries end in an error, a permission denial occurs, or the hard timeout expires without usable file or diff progress.
...
The primary Claude agent must still make the final decision and must independently verify the real path, diff, logs, and relevant checks. Never accept a subagent self-report alone.
