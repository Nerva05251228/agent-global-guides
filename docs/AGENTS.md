...
- `$codex-subagent-orchestration`: Codex-primary subagent routing, task specs, parallel/serial dispatch, read-only/editable invocation, stream logs, timeouts, retries, and fallback.
...
For each long or complex authoritative-plan item, record the executor (`Primary Codex`, `Codex subagent`, `Claude subagent`, or another named agent), dispatch reason, and verification owner. This metadata is not mandatory for ordinary short tasks.
