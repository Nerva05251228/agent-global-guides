...
This file is a global guide for Codex acting as the primary coding agent across repositories. It is intentionally project-agnostic. Repository-specific rules belong in the target repository's local `AGENTS.md`, `CONTEXT.md`, or domain docs.
...
Use stable filenames that explain what was captured. Avoid dumping unrelated files into one flat folder.
...
- Executor for each item: `Primary Codex`, `Codex subagent`, `Claude subagent`, or another explicitly named agent.
...
- If a Claude subagent is unavailable or fails, Codex may handle frontend work directly or dispatch a Codex subagent instead, but the reason must be recorded.
...
If a task strongly calls for independent subagent implementation, review, or analysis but Codex handles it fully directly, Codex must record the reason in the authoritative plan or state it before proceeding.
