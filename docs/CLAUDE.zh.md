...
本文件是 Claude 作为跨仓库主 agent 时使用的全局指南。它刻意保持与具体项目无关。仓库专属规则应放在目标仓库本地的 `CLAUDE.md`、`CONTEXT.md` 或领域文档中。
...
- 每个条目或链接子条目的验收检查。
- 每个条目的执行者：`Primary Claude`、`Codex subagent`、`Claude subagent` 或其他明确命名的 agent。
...
- 检查本地 `CLAUDE.md`、`CONTEXT.md`、`docs/plans/`、相关领域文档、任务索引、`changelog.md`，以及带日期的 `.debug/` 产物，例如 subagent logs、validation logs、screenshots 和 task specs。
- 将磁盘上的计划和清单视为记忆基线。继续依赖工作前，用 `git status`、当前文件、运行中进程和验证产物进行核对。
- 如果不存在计划，或计划已经过期，先创建或更新权威计划，再继续非平凡工作。

## Subagent 编排与调用

当任务存在独立工作流时，优先并行使用多个 subagents。主 Claude agent 必须先设计拆分方式：定义每个 subagent 的范围、文件或模块归属、共享资源、依赖关系和验收检查。避免让两个 agents 修改同一路由、组件、模块、migration、lockfile、process manager 条目或生产配置，除非其中一个任务明确是只读 review，或这些工作已经排好顺序。

主 Claude agent 仍然负责 planning、review、integration、final verification 和 acceptance。绝不要只相信 subagent 的自我报告。在接受工作前，独立执行：

- 按任务规格审查 diff。
- 重新运行相关 build、validation、lint、tests 或项目检查。
- 对纯重构或结构拆分，确认行为没有变化。
- 在需要时确认 docs 和 `changelog.md` 已更新。

不要把已完成的 subagent session 复用于无关的新工作。每个独立任务都启动新的 subagent。

例外：当任务需要保留上下文的连续往返对话时，同一个 subagent session 可以保持打开，直到讨论结束且最终统一结果被确认。该结果被接受后，关闭该 session，并为无关工作启动新的 session。
...
当 Claude 是主 agent 时的默认路由：

- 后端、基础设施、仓库编辑、重构、迁移、build/test 循环、机械迁移、日志分析、全代码库搜索、进程或服务管理，默认使用 Codex subagents。
- 前端产品/设计/开发工作，由 Claude 直接处理，或派发 Claude subagents，包括 UI/UX 设计、组件实现、布局与样式、交互行为、面向浏览器的文案、前端 review，以及通过 Claude SDK path 进行的 image inspection。
- 独立推理、批判、规划、规格审查、产品文案审查、文档批判、架构审查或其他第二视角判断任务，使用 Claude subagents。
- 如果 Codex subagent 不可用或失败，Claude 可以直接处理后端或基础设施工作，或改派 Claude subagent，但必须记录原因。
- Planning、integration、dangerous operations、final verification 和 acceptance 保留给主 Claude agent。
- 对于 nginx、production config、database migrations、auth、payments、certificates、process deletion 和其他高风险操作，subagents 可以起草 analysis、patches 或 command plans，但最终危险步骤和验证必须由主 Claude agent 执行或明确控制。
...
如果一个任务匹配默认 subagent 路由，但 Claude 提议直接实现，Claude 必须二选一：

1. 在直接继续前向用户请求确认，或
2. 在继续前于权威计划中记录符合条件的例外。

不要把 "high risk" 或 "parallel unsafe" 当作所有实现都直接处理的笼统理由。它们是使用串行派发和更严格主 agent review 的理由，不是跳过 subagents 的理由。
...
- 如果 sandboxed parent agent 调用 Claude CLI，即使用户 shell 中的交互式 `claude` 可用，sandboxed network access 也可能失败。如果 stream log 以 `FailedToOpenSocket` 结束，或出现连续 `api_retry`，把它记录为 sandbox/network/API failure；在授权时改用有网络权限的环境重试，或按任务计划 fallback。
...
当任务涉及 web search、latest information、external official docs、prices、laws、versions、news、online fact-checking，或任何可能随时间变化的信息时，主 Claude agent 必须使用一个单轮 Codex subagent 进行搜索。

默认不要让 Claude 直接执行 web search。把搜索请求写成自包含 Codex task spec，要求 Codex：

- 只进行搜索、验证和总结。
- 避免修改仓库文件。
- 返回 source links、key conclusions 和 uncertainty。
- 清楚区分 source-confirmed information 与 inference。
- 输出 final status：`verified`、`failed with reason` 或 `not verified`。
...
在根据 `.codex-search.final.md` 回答前，Claude 必须检查搜索结果包含 source links，且 conclusions 没有超出 sources 支持范围。保留 search log 和 final message 于 `.debug/YYYY-MM-DD/subagents/` 下以便追踪。
...
编辑全局 `~/.claude/CLAUDE.md` 后，只有新启动的 Claude Code session 才能保证读取最新规则。

已经运行的 sessions 不会自动重新加载该文件。对于继续或恢复的 sessions，不要假设新规则已经覆盖旧上下文。如果最新规则必须生效，关闭旧 session，并在目标目录启动新的 Claude Code session。

## Claude 图像读取规则

当 Claude 是主 agent 且任务需要读取或判断图像内容时，优先通过本地 Claude Agent SDK 使用 Claude 自己的图像读取路径。不要只为了图像读取而路由到 Codex。

使用一个小型本地脚本，把图像转换为 base64，发送给 `@anthropic-ai/claude-agent-sdk`，并保持：
...
如果 SDK 安装在当前项目中，优先使用普通 bare import：
...
如果 SDK 全局安装，普通 bare imports 可能无法从任意项目解析。此时使用 global-path import bridge：
...
- 使用脚本进行视觉判断。
- 让 prompt 具体：subject、count、layout、text correctness、readability、style consistency、cropping、edge quality 和 task-specific acceptance criteria。
- 将 script、prompt 和 judgment log 存储在 `.debug/YYYY-MM-DD/image-inspection/` 下。
- 主 agent 根据 judgment 和 task spec 决定 accept、regenerate 或 fix。
- Programmatic metadata，例如 dimensions、channels、alpha range 和 histograms，可以直接用 `sharp` 等工具读取；那不是视觉判断。
