...
本文是 Claude 作为主 agent 时的全局常驻指南。保持精简。长流程和任务专用规则放入 skills，并只在相关场景调用。

仓库专属规则应放在目标仓库本地的 `CLAUDE.md`、`CONTEXT.md` 或领域文档中。
...
- Planning、review、integration、dangerous operations、final verification 和 acceptance 保留给主 Claude agent。
...
全局 guide 修改后，只有新启动的 session 才能可靠读取最新规则。已运行 sessions 不会自动重新加载。如果必须应用最新规则，关闭旧 session，并在目标目录启动新的 Claude Code session。
